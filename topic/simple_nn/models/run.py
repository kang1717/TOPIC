...
def train_model(inputs, logfile, model, optimizer, criterion, scale_factor, pca, device, best_loss, train_loader, valid_loader, atomic_e=False):
    ...
    for epoch in tqdm(range(inputs['neural_network']['start_epoch'], total_epoch+1), unit='epoch'):
        ...
        ############# New code ##################
        early_stop = early_stopping.check_early_stop_condition(inputs, logfile, epoch, train_epoch_result, valid_epoch_result, early_stop_dict)
        if early_stop == True:
            save_checkpoint(epoch, loss, model, optimizer, pca, scale_factor, filename='checkpoint_early_stop.pth.tar')
            model.write_lammps_potential(filename='./potential_saved_early_stop', inputs=inputs, scale_factor=scale_factor, pca=pca)
            logfile.write("checkpoint_early_stop.pth.tar & potential_saved_early_stop written\n")
            break
        #########################################
...
def _initialize_test_result_dict(inputs):
    res_dict = {'DFT_E': list(), 'NN_E': list(), 'N': list(), 'struct_type': list()}
...
def _update_calc_results_in_results_dict(n_batch, res_dict, item, calc_results, use_force, use_stress, atomic_e=False):
    for n in range(n_batch):
        ...
        res_dict['struct_type'].append(item['struct_type'][n])
