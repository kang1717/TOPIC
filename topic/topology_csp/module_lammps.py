from topic.topology_csp.basic_tools import direct2cartesian
import numpy as np
import os, sys, yaml
...
def lammps_write(pos, total_yaml, cation_dict):
    ################# Read total_yaml ##################
    material    = total_yaml['material']
    #bond_dict   = total_yaml['distance_constraint']
    bond_dict   = total_yaml['LJ_constraint']
    repulsion_dict   = total_yaml['repulsion_constraint']
    second_dict = total_yaml['distance_constraint_2']
    lj_factor = total_yaml['lj_distance_factor']
    harmonic_factor = total_yaml['harmonic_distance_factor']
    ...
    # bond distance
    O_O     = repulsion_dict["O-O"] * harmonic_factor
    m1_m1   = repulsion_dict[f"{cat_info[0][0]}-{cat_info[0][0]}"]*harmonic_factor
    m2_m2   = repulsion_dict[f"{cat_info[1][0]}-{cat_info[1][0]}"]*harmonic_factor
    if f"{cat_info[0][0]}-{cat_info[1][0]}" in repulsion_dict:
        m1_m2 = repulsion_dict[f"{cat_info[0][0]}-{cat_info[1][0]}"]*harmonic_factor
    else:
        m1_m2 = repulsion_dict[f"{cat_info[1][0]}-{cat_info[0][0]}"]*harmonic_factor
    ...
    lj_bond = {key: value*lj_factor/(2**(1/6)) for key, value in bond_dict.items()}
    ####################################################
    ...
    with open("in.add1","w") as lfile:
        for cat in cation_dict:
            c_dict = cation_dict[cat]
            for an in c_dict:
                if [an,cat] not in pair_lists and [cat,an] not in pair_lists:
                    bond = lj_bond[pos['atomarray'][cat]+'-'+pos['atomarray'][an]]
                    fixnum += 1
                    lfile.write(f"pair_coeff {cat+1} {an+1} lj/cut 3.0 {bond} 6.0\n")
                    pair_lists.append([cat,an])
...
def pos_dict2cooall(pos, output='coo'):
    ...
    alpha = angle(latt[1],latt[2]); alphar = alpha/rad2deg;
    beta  = angle(latt[0],latt[2]); betar  = beta/rad2deg;
    gamma = angle(latt[0],latt[1]); gammar = gamma/rad2deg;
...
def coo2pos_dict(file_path, total_yaml):
    material = total_yaml['material']
    ...
    for i, line in enumerate(lines):
        ...
        # Detect the beginning of the atom section
        if "Atoms" in line:
            atom_section = True
        ...
        if atom_section and len(line.split()) > 2:
            ...
            #atomic_numbers.append(atom_type-1)  # Store atom type
            #atomic_elements.append(pos['atomlist'][atom_type-1])  # Store atom type
...
def change_coo_index(file_path, pos):
    ...
    with open(file_path, 'w') as s:
        ...
        for i, line in enumerate(lines):
            if i < 10:
                if i == 3:
                    s.write(f"{len(pos['numlist'])} atom types\n")
                else:
                    s.write(line)
            ...
            if atom_section and len(line.split()) > 2:
                tokens = line.split()
                atom_id = int(tokens[0])  # Atom ID (not used)
                atom_type = pos['numarray'][atom_id-1] + 1
                x, y, z = tokens[2:5]  # Atom positions
                s.write(f"{atom_id} {atom_type} {x} {y} {z}\n")
