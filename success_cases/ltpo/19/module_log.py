def write_log_head():
  ...
  with open("log","w") as w:
      ...
      w.write("     Iter_for_LJ/harmonic     Iter_for_NNP     Struct_gen_time  LJ_success   LJ_time  NNP_success  NNP_time  NNP_time2     Energy       Volume        T\n")
...
def write_log(iteration_1, iteration_2, T0, T1, T2, T3, fail_2, fail_3, E, V):

  with open("log","a") as w:

    T = T0 +T1 + T2 + T3

    w.write(f"         {iteration_1:5}                   {iteration_2}                {T0:.2f}             {fail_2:2}         {T1:.2f}        {fail_3:3}        {T2:.2f}     {T3:.2f}         {E:4.2f}      {V:4.2f}      {T:.2f}\n")
